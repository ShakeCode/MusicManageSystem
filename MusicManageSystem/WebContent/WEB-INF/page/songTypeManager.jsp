...
<html lang="zh-CN">
		...
		<div class="container-fluid">
			<div class="row">
				...
				<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main" ng-controller="UserList">
					<h2 class="sub-header">歌曲类型列表</h2>
					<div>
						<a href="#" class="btn btn-primary btn-xm col-md-2" ng-click="addNewUser()">
							<span class="glyphicon glyphicon-plus">添加歌曲类型</span>
						</a>
								...
								<div class="modal-body">
									<form id="defaultForm">
										...
										<div class="form-group input-group">
											...
											<input type="text" class="form-control" placeholder="请输入歌曲类型" ng-model="modalName" name="songname">
										</div>
									</form>
								</div>
	...
	<script>
		...
		$(document).ready(function() {
			...
			var u = <%=request.getAttribute("songTypes")%>;
			var l = u.length;
			if(l % pagenums == 0) {
		...
		var userlist = <%=request.getAttribute("songTypes")%>;
		...
		app.controller("UserList", function($scope) {
			...
			$scope.search = function() {
				...
				var index = 0;
				for(var user in userlist) {
					if(userlist[user].typeName.indexOf(con) >= 0) {
			...
			$scope.modify = function(index) {
				...
				$scope.modalName = showlist[index].typeName ;
